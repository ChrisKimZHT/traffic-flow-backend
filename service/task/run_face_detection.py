...
run_face_detection_router = APIRouter()


@run_face_detection_router.get("/runFaceDetection")
async def _(videoId: int, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    output_path = os.path.join(cur_work_dir, f"runs/face/{file_name_no_ext}")
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    cmd = (f"cd \"{config.face_detection_path}\" && "
           f"\"{config.face_detection_interpreter}\" \"{config.face_detection_main}\" "
           f"--input_video \"{input_video}\" "
           f"--output_path \"{output_path}\" "
           f"--log_file log.txt")

    try:
        task = TaskCrud.create(db, videoId, f"人脸分析", int(time.time()), os.path.join(output_path, "log.txt"), 1)
        video.face_processed = 1
        video.face_res_path = output_path
    ...
    def callback(return_value):
        try:
            ...
            if return_value == 0:
                cb_task.status = 2
                cb_video.face_processed = 2
            else:
                cb_task.status = 3
                cb_video.face_processed = 0
